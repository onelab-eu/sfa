...
def bonfire_authenticate():
    ...
    h["location"]  = parser.get('bonfire', 'url')
    return h  
...
def show_slice_credential(slice_name):
    ...
    root = tree.getroot()
    hashres = {}
    hashres["slice_native"] = root.findall(".//signatures//{http://www.w3.org/2000/09/xmldsig#}Signature//{http://www.w3.org/2000/09/xmldsig#}KeyInfo//{http://www.w3.org/2000/09/xmldsig#}X509Data//{http://www.w3.org/2000/09/xmldsig#}X509SubjectName")[0].text
    hashres["X509IssuerName"] = root.findall(".//signatures//{http://www.w3.org/2000/09/xmldsig#}Signature//{http://www.w3.org/2000/09/xmldsig#}KeyInfo//{http://www.w3.org/2000/09/xmldsig#}X509Data//{http://www.w3.org/2000/09/xmldsig#}X509IssuerName")[0].text
    for target in root.findall('credential'):
        hashres["slice_user_urn"] = target.find('owner_urn').text
        hashres["slice_urn"] = target.find('target_urn').text
        hashres["serial"] = target.find('serial').text
    return hashres

""" create a bonfire experiment from a sfa point of view """
def allocate(user_name, groups, description, walltime, slice_name):
    hashres = {}
    hashres = show_slice_credential(slice_name)
    urlexperiment = parser.get('bonfire', 'url') + "experiments"
    create_fed4fire_exp(user_name, groups, description, walltime, hashres["slice_urn"], hashres["slice_user_urn"], hashres["slice_native"], urlexperiment)
...
def new_user_slice():
    ...
    """ slicenfo = n["url"] + "." + n["name"] + "_" + n["name"] """
    sliceinfo = "topdomain.dummy." + n["name"] + "_slice"
    txtslice = "sfaadmin.py reg register -x {0} -t slice -r {1}".format(sliceinfo, url)
...
def create_vm(testbed, nb_experiment, name_compute, groups):
    url = parser.get('bonfire', 'url') + 'experiments/' + num_experiment + '/computes' 
    if testbed == "fr-inria":
...
def stop_vm(testbed, num_compute):
    url = parser.get('bonfire', 'url') + "locations/" + testbed + "/computes/" + num_compute
...
def provisioning(num_experiment, status):
    url = parser.get('bonfire', 'url') + "experiments/" + num_experiment
...
def callcurl(url):
    ...
    """ with the authentication against the ldap, peuhaps these two ligns below are not necessary """  
...
def bonsources():
    ...
    urlnetworks = buildpagehttp(parser.get('bonfire', 'url') + "locations/", "networks", locations)
    urlstorages = buildpagehttp(parser.get('bonfire', 'url') + "locations/", "storages", locations)
    urlcomputes = buildpagehttp(parser.get('bonfire', 'url') + "locations/", "computes", locations)
    """ main code """
    bonfires = []
    ...
    manag_exp =  remove_needless_txt(callcurl(parser.get('bonfire', 'url') + "managed_experiments"))
